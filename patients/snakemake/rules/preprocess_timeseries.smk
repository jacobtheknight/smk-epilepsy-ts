# Sample VEP atlas and extract patient BOLD time series adding SEEG-derived epileptic network labels
...
rule resample_vep_atlas_to_mni:
    ...
    output:
        atlas_mni = join(config["patient_dir"], 'data/{subject}/{subject}_task-rest_dir-{dir}_run-{run}_space-MNI152NLin2009cAsym_desc-vep_dseg.nii.gz'),
...
rule check_atlas_alignment:
    ...
    output:
        validation_nifti = join(config["patient_dir"], 'qc/{subject}/{subject}_task-rest_dir-{dir}_run-{run}_atlas_alignment.nii.gz'),
        validation_png = join(config["patient_dir"], 'qc/{subject}/{subject}_task-rest_dir-{dir}_run-{run}_atlas_alignment.png'),
...
rule preprocess_timeseries_vep:
     ...
     input:
         ...
         vep_atlas = join(config["patient_dir"], 'data/{subject}/{subject}_task-rest_dir-{dir}_run-{run}_space-MNI152NLin2009cAsym_desc-vep_dseg.nii.gz'),
         lut = join(config["vep_lut"]),
         validation = join(config["patient_dir"], 'qc/{subject}/{subject}_task-rest_dir-{dir}_run-{run}_atlas_alignment.nii.gz'), 
     output:
         checkpoint_timeseries = join(config["patient_dir"], 'data/{subject}/{subject}_task-rest_dir-{dir}_run-{run}_timeseries_checkpoint.csv'),
         vep_timeseries = join(config["patient_dir"], 'data/{subject}/{subject}_task-rest_dir-{dir}_run-{run}_vep_timeseries.csv'),
...
rule seeg_zone_label:
     """Map epileptogenic, propagator, non-involved and non-explored zone labels from SEEG to VEP BOLD time series"""
     input:
         vep_timeseries = join(config["patient_dir"], 'data/{subject}/{subject}_task-rest_dir-{dir}_run-{run}_vep_timeseries.csv'), 
         ei_file = join(config["ei_dir"], '{subject}_EI.xlsx')
     output: 
         vep_timeseries_labelled = join(config["patient_dir"], 'data/{subject}/{subject}_task-rest_dir-{dir}_run-{run}_vep_timeseries_labelled.csv'),
         done = 'logs/{subject}/{subject}_task-rest_dir-{dir}_run-{run}_label_vep_timeseries.done'
     group: "preprocess_timseries"
     script: join(config["snakemake"], 'scripts/preproc/preproc_zone_label.py')
