# Parcellate streamline-weighted connectomes from pre-computed SIFT anatomically-constrained tractography using MRtrix3 (PATIENTS)
...
rule transform_t1w_to_dwi:
  ...
  input:
    t1w_nii = join(config["freesurfer_dir"], '{subject}/mri/T1.nii.gz'), 
    dwi_nii = join(config["diffusion_dir"], '{subject}/{subject}_b0_mean_blip.nii.gz'), 
  output:
...
rule check_atlas_alignment:
  ...
  input:
    tracto_vis = join(config["diffusion_dir"], '{subject}/tractography/vis.nii.gz'), 
    vep_atlas_diff = join(config["connectome_dir"], '{subject}/work/aparc+aseg.vep_diff.mif')
  output:
...
rule parcellate_tracks: 
  """Parcellate tracks and compute weighted connectomes (number of streamlines normalised by region size)"""
  input: 
    tracks = join(config["diffusion_dir"], '{subject}/tractography/{subject}_10M_act_tracks_sift.tck'), 
    vep_atlas_diff = join(config["connectome_dir"], '{subject}/work/aparc+aseg.vep_diff.mif')
  output:
